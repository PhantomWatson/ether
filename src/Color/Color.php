...
{
    /**
     * To match with an XKCD color, a given color has to be within this threshold for its R, G, and B values
     * @var int
     */
    ...
    /**
     * Returns an array of all colors from the XKCD colors survey (https://xkcd.com/color/rgb/)
     *
     * @return array
     */
    public function getXkcdColors()
    {
        if (!empty($this->colors)) {
            return $this->colors;
        }

        // A dirty hack while I'm playing around with this idea
        $colorsString = '
            cloudy blue	#acc2d9	
    ...
    public function getClosestXkcdColor($color)
    {
        ...
        foreach ($xkcdColors as $xkcdColorName => $xkcdColorParts) {
            ...
            foreach (['r', 'g', 'b'] as $component) {
                $colorDifference = abs($colorParts[$component] - $xkcdColorParts[$component]);
                if ($colorDifference <= $this->threshold) {
                    $proximity += $colorDifference;
                } else {
                    $isCandidate = false;
                    break;
                }
            ...
            if ($isCandidate) {
                $candidates[$xkcdColorName] = $proximity;
            }
        }
